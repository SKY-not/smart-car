...
ros::Subscriber sub;
int id;
double rad[key_points],x_rad,y_rad,x[key_points]={1,3,5},y[key_points]={0,0,0},x_rad2,y_rad2;
...
void get_rad_xy(){
    // 这个函数是错的，假定了第二个辐射源的位置和系数
    x_rad2=0;
    y_rad2=-5.7;
    for(int i=0;i<key_points;++i)
        rad[i]=rad[i]-10/dist_sqr(x_real[i],y_real[i],x_rad2,y_rad2);
    double ans=100;
...
int main(int argc, char** argv){
    ros::init(argc, argv, "simple_navigation_goals2");
    ros::NodeHandle nh;
    sub=nh.subscribe<std_msgs::Float64MultiArray>("radiation2",20,radiation_sub);
    // 建立通信
    ...
    FILE *fout=fopen("/home/ucar/ucar_ws/rad_info2.txt","w");
    fprintf(fout,"%lf %lf %lf %lf\n",x_rad,y_rad,x_rad2,y_rad2);
    fclose(fout);

    goal.target_pose.pose.position.x = x_rad2;
    goal.target_pose.pose.position.y = y_rad2;
    ROS_INFO("meow %lf %lf %lf %lf\n",x_rad,y_rad,x_rad2,y_rad2);
    ac.sendGoal(goal);
    ac.waitForResult();
    return 0;
}
