...
class navigation_node2:
    ...
    def __init__(self):
        ...
        meow("navigation2.py cmd_vel pub")
        ...
        while not rospy.is_shutdown():
            current_position = rospy.wait_for_message('/current_position', Float64MultiArray, timeout=None)
            current_x = current_position.data[0]
            current_y = current_position.data[1]

            # Check if the robot is close enough to the target position
            distance = np.sqrt((self.goal.target_pose.pose.position.x - current_x) ** 2 +
                       (self.goal.target_pose.pose.position.y - current_y) ** 2)
            if distance < 0.1:  # Threshold for reaching the target
                break

            # Publish the control command
            twist_msg = Twist()
            twist_msg.linear.x = self.goal.target_pose.pose.position.x - current_x
            twist_msg.linear.y = self.goal.target_pose.pose.position.y - current_y
            twist_msg.angular.z = 0.0  # Assuming no rotation is needed
